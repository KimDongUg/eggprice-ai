  ...
  useEffect(() => {
    api.get("/news", { params: { limit: 10, category: "가격" } })
      .then((r) => {
        ...
        const recent = (r.data.items || []).filter(
          (item: { published_at: string }) => new Date(item.published_at).getTime() >= weekAgo
        );
        setNews(recent.slice(0, 3));
      })
      .catch(() => {});
  }, []);
          ...
          <div className="space-y-4">
            {[
              ...
              },
              {
                ...
                a: "KAMIS 농산물유통정보의 실시간 시세 데이터, 사료 가격(옥수수·대두박), 환율, 기온, 조류인플루엔자, 계란 수입량·수입단가 등 다양한 시장 요인을 종합 분석합니다.",
              },
              {
                q: "해지는 언제든 가능한가요?",
